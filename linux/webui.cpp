...
static void SetValue(const char* msg, int val)
{
    for (int i = 0; i < 6; ++i) {
        if (strcmp(f_values[i].msg, msg) == 0) {
            f_values[i].value = val;
            break;
        }
    }
}

static void ParseMessage(const char* pMessage)
{   
    while (*pMessage != 0) {
        int i = 0;
        char msg[256];
        ...
        while (*pMessage != ':' && *pMessage != 0) {
            msg[i++] = *pMessage++;
        }
        msg[i] = 0;
        
        value = atoi(++pMessage);
        while(*pMessage != 0 && *pMessage++ != ';');
...
onopen(libwebsock_client_state *state)
{
  ...
  char buffer[256];
  sprintf(buffer, "hMin:%i;hMax:%i;sMin:%i;sMax:%i;vMin:%i;vMAx:%i;", 
    f_values[0].value, f_values[1].value, f_values[2].value, f_values[3].value, f_values[4].value, f_values[5].value);
  
  libwebsock_send_text(state, buffer);
  return 0;
}
